...
TARGET_ROLE_ID         = 1488590682342953163   # for /dm command
...
async def vouch_add(interaction: discord.Interaction, user: discord.Member, amount: int):
    ...
    vouches.setdefault(user.id, [])
    # Use negative IDs to mark manual additions (won't clash with real Discord user IDs)
    ...
    embed.description = (
        ...
        f"They now have **{count}** total vouch{'es' if count != 1 else ''}."
    )
    embed.set_footer(
        text=f"{FOOTER_HELP} • Today at {discord.utils.utcnow().strftime('%I:%M %p')}")
    await interaction.response.send_message(embed=embed)
