...
# ─────────────────────────────────────────────
#  VOUCH STORAGE  (persisted to vouches.json)
#  vouches[user_id] = list of voucher_ids
# ─────────────────────────────────────────────
VOUCH_FILE = pathlib.Path("vouches.json")

def load_vouches() -> dict[int, list[int]]:
    if VOUCH_FILE.exists():
        raw = json.loads(VOUCH_FILE.read_text())
        return {int(k): v for k, v in raw.items()}
    return {}

def save_vouches() -> None:
    VOUCH_FILE.write_text(json.dumps(vouches))

vouches: dict[int, list[int]] = load_vouches()
...
class AppView(discord.ui.View):
    ...
    async def open_ticket(self, interaction: discord.Interaction, btn: discord.ui.Button):
        ...
        for ch in cat.channels:
            if ch.name == f"app-{interaction.user.name}" or ch.topic == str(interaction.user.id):
                await interaction.followup.send(
...
async def vouch(interaction: discord.Interaction, user: discord.Member, reason: str = None):
    ...
    # Only /vouch posts to the vouch log channel
...
async def vouch_add(interaction: discord.Interaction, user: discord.Member, amount: int):
    ...
    # Actually update the vouch count and persist it
    vouches.setdefault(user.id, [])
    # Use 0 as a sentinel for manually added vouches (no real voucher ID)
    vouches[user.id].extend([0] * amount)
    save_vouches()
    new_count = len(vouches[user.id])

    embed = discord.Embed(color=0x57f287, title="✅ Vouches Added")
    embed.description = (
        f"Added **{amount}** vouch{'es' if amount != 1 else ''} to {user.mention}.\n"
        f"They now have **{new_count}** vouch{'es' if new_count != 1 else ''} total."
    )
    embed.set_footer(
        text=f"{FOOTER_HELP} • Today at {discord.utils.utcnow().strftime('%I:%M %p')}")
    # Reply only to the invoker — does NOT post to the vouch log channel
    await interaction.response.send_message(embed=embed)
...
# ─────────────────────────────────────────────
#  ENTRY POINT  (with Discord 500 / session retry logic)
# ─────────────────────────────────────────────
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            print("⚠️  Discord returned a 500 error. Retrying in 60 seconds...")
            time.sleep(60)
            # Restart the process cleanly so aiohttp session is fully fresh
            os.execv(sys.executable, [sys.executable] + sys.argv)
        except Exception as e:
            msg = str(e)
            if "Session is closed" in msg or "Cannot connect" in msg:
                print(f"⚠️  Connection error ({msg}). Retrying in 30 seconds...")
                time.sleep(30)
                os.execv(sys.executable, [sys.executable] + sys.argv)
